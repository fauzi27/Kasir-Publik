...
export default function SuperAdmin({ currentUser, onImpersonate }) {
  ...
  // 🔥 STATE BARU UNTUK METRIK BANDWIDTH
  const [totalGlobalTx, setTotalGlobalTx] = useState(0); 
  const [isLoading, setIsLoading] = useState(true);
  const [searchTerm, setSearchTerm] = useState('');

  // === AMBIL SEMUA DATA & HITUNG METRIK REAL-TIME ===
  const fetchAllData = async () => {
    ...
    try {
      ...
      let staffMap = {}; 
      let staffCount = 0;
      let globalTxCount = 0; // Akumulator beban database

      // 1. Pisahkan Bos dan Karyawan
      ...
      // 2. Hitung penggunaan kuota bulan ini + Beban Global
      const enrichedOwnerList = await Promise.all(ownerList.map(async (owner) => {
        let currentUsage = 0;
        try {
          // Selalu hitung untuk memantau beban, meski kuotanya Unlimited
          ...
          globalTxCount += currentUsage; // Tambahkan ke beban total server
        } catch (err) { 
          console.error("Gagal hitung kuota", err); 
        }
      ...
      setTotalGlobalTx(globalTxCount); // Simpan metrik bandwidth
    } catch (error) {
  ...
  const handleLogout = () => {
    Swal.fire({
      title: 'Keluar dari ISZI?', icon: 'question', showCancelButton: true,
      confirmButtonColor: '#ef4444', confirmButtonText: 'Ya, Logout'
  ...
  return (
    <div className="flex flex-col h-screen bg-slate-950 text-slate-200 font-sans overflow-hidden">
      ...
      <div className="flex-1 overflow-y-auto p-6">
        ...
        {/* 🔥 KARTU METRIK DENGAN BANDWIDTH METER 🔥 */}
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
          ...
          <div className="bg-slate-900 border border-slate-800 p-5 rounded-2xl shadow-sm relative overflow-hidden">
            <div className="absolute top-0 right-0 w-16 h-16 bg-emerald-500/10 rounded-bl-full flex items-start justify-end p-3"><i className="fas fa-database text-emerald-500 text-lg"></i></div>
            <p className="text-xs text-slate-400 font-bold uppercase tracking-wider mb-1">Beban Database (Bulan Ini)</p>
            <h2 className="text-4xl font-black text-emerald-400">{totalGlobalTx.toLocaleString()}</h2>
            <p className="text-[10px] text-slate-500 font-mono mt-1">Total Transaksi / Write Load</p>
          </div>
          
          <div className="bg-slate-900 border border-slate-800 p-5 rounded-2xl shadow-sm flex flex-col justify-center">
            <p className="text-xs text-slate-400 font-bold uppercase tracking-wider mb-2">Status Operasional</p>
            <div className="flex items-center gap-4">
              <div><span className="text-xl font-black text-blue-400">{activeCount}</span> <span className="text-[10px] text-slate-500">Aktif</span></div>
              <div><span className="text-xl font-black text-red-400">{suspendedCount}</span> <span className="text-[10px] text-slate-500">Blokir</span></div>
              <div className="ml-auto flex items-center gap-1.5">
                <span className="relative flex h-3 w-3"><span className="animate-ping absolute inline-flex h-full w-full rounded-full bg-emerald-400 opacity-75"></span><span className="relative inline-flex rounded-full h-3 w-3 bg-emerald-500"></span></span>
                <span className="text-emerald-400 font-bold text-xs tracking-widest">ONLINE</span>
              </div>
            </div>
          </div>
        </div>
                ...
                {isLoading ? (
                  ...
                  filteredOwners.map((owner) => (
                    <tr key={owner.id} className="hover:bg-slate-800/30 transition-colors">
                      ...
                      {/* KOLOM PROFIL & KARYAWAN (KLIKABLE) */}
                      <td className="p-4">
                      ...
                      <td className="p-4">
                        ...
                        <div className="flex flex-col gap-1.5">
                          ...
                          <span className="text-[10px] font-mono px-2 py-0.5 rounded border bg-slate-950 text-slate-400 border-slate-800 flex items-center gap-2 max-w-max">
                            <i className="fas fa-hamburger w-3 text-center"></i> Menu: {owner.maxMenus > 0 ? `Max ${owner.maxMenus}` : '∞ (Unlimited)'}
                          </span>
                          <span className="text-[10px] font-mono px-2 py-0.5 rounded border bg-slate-950 text-slate-400 border-slate-800 flex items-center gap-2 max-w-max">
                            <i className="fas fa-image w-3 text-center"></i> Foto: {owner.maxImages > 0 ? `Max ${owner.maxImages}` : '∞ (Unlimited)'}
                          </span>
                        </div>
                      </td>
                      ...
                      <td className="p-4 flex flex-wrap justify-center gap-2">
                        {onImpersonate && (
                          <button onClick={() => onImpersonate(owner.id)} className="w-9 h-9 rounded-xl bg-purple-900/30 text-purple-400 border border-purple-800/50 hover:bg-purple-800/50 hover:text-white flex items-center justify-center transition shadow-sm" title="Mata Dewa (Masuk Toko)">
                            <i className="fas fa-user-secret"></i>
                          </button>
                        )}
                        <button onClick={() => handleEditLimit(owner)} className="w-9 h-9 rounded-xl bg-blue-900/30 text-blue-400 border border-blue-800/50 hover:bg-blue-800/50 hover:text-white flex items-center justify-center transition shadow-sm" title="Atur Kuota Limit">
